...
for (;;) {
    ...
    if (stream_select($read, $write, $except, $timeout)) {
        ...
        error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 020 ${line_number}");
        break;
    }
}
...
error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 030 " . strlen($request_data));
...
error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 050 " . count($distccds));

for (;;) {
    foreach ($distccds as &$distccd) {
        error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 060 ${distccd}");
        ...
        if ($mc->get("DISTCCD_URL_${distccd}") < 4) {
            ...
            error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 070 ${distccd}");
        }
    }
    if ($target != '') {
        break;
    ...
    sleep(3);
    error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 080");
}

error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 090 ${target}");
...
error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 130 " . strlen($res));

$http_code = (string)curl_getinfo($ch, CURLINFO_HTTP_CODE);

error_log(date("Y-m-d H:i:s") . " ${pid} send.php check point 140 ${http_code}");
